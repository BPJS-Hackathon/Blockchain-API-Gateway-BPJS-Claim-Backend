...
func Faskes1Only() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !exists || role != models.RoleFaskes {
			c.JSON(http.StatusForbidden, gin.H{
				"success": false,
				"message": "Access restricted to Faskes users only",
			})
			c.Abort()
			return
		}

		// Check jika user adalah faskes1 berdasarkan username
		username, exists := c.Get("username")
		if !exists || username != os.Getenv("FASKES1") {
			c.JSON(http.StatusForbidden, gin.H{
				"success": false,
				"message": "Access restricted to Faskes1 users only",
			})
			c.Abort()
			return
		}
...
func Faskes2Only() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Check jika user adalah faskes2 berdasarkan username
		username, exists := c.Get("username")
		if !exists || username != os.Getenv("FASKES2") {
			c.JSON(http.StatusForbidden, gin.H{
				"success": false,
				"message": "Access restricted to Faskes2 users only",
			})
			c.Abort()
			return
		}
